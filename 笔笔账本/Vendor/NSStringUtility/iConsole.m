...
- (void)setConsoleText
{
	...
	int touches = (int)(TARGET_IPHONE_SIMULATOR ? _simulatorTouchesToShow: _deviceTouchesToShow);
	...
	if (![textField.text isEqualToString:@""])
	{
        ...
        if (nil != _delegate)
        {
            if ([_delegate respondsToSelector:@selector(handleConsoleCommand:)]/* ||
                [(Class)_delegate instancesRespondToSelector:@selector(handleConsoleCommand:)]*/)
            {
                // TODO 实现自带命令: 切换level等
