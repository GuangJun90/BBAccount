...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
        NSString *pinyin = [((ChineseContenr*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((ChineseContenr*)object).pinYin substringToIndex:1];
        NSString *string = ((ChineseContenr*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseModelArr:(NSArray*)stringArr
{
    ...
    {
        id model = [stringArr objectAtIndex:i];
        ChineseContenr *chineseString = [[ChineseContenr alloc]init];
        
        chineseString.string = [NSString stringWithString:[model valueForKey:@"sortChinese"]];
        ...
        chineseString.string = [ChineseContenr RemoveSpecialCharacter:chineseString.string];
...
//            NSLog(@"chineseString.string== %@",chineseString.string);
        ...
        [model setValue:chineseString.pinYin forKey:@"pinYin"];
...
+(NSMutableArray *)LetterSortModelByPinYin:(NSArray *)modelArray{
    ...
    NSMutableArray *tempArray = [self ReturnSortChineseModelArr:modelArray];
    ...
    for (id model in tempArray) {
        ...
        NSString *pinyin = [[model valueForKey:@"pinYin"] substringToIndex:1];
        // NSString *string = ((ChineseContenr*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
...
{
    NSMutableArray *tempArray = [self ReturnSortChineseArr:[stringArr mutableCopy]];
    NSMutableArray *LetterResult = [NSMutableArray array];
    NSMutableArray *item = [NSMutableArray array];
    NSString *tempString;
    //拼音分组
    for (ClientsModel* object in tempArray) {
        DLog(@"%@%@",object,object.pinYin);
        NSString *pinyin;
        if ([object.pinYin isNotBlank]) {
...
{
    NSMutableArray *tempArray = [self ReturnSortChineseCustomerManageArr:[stringArr mutableCopy]];
    NSMutableArray *LetterResult = [NSMutableArray array];
    NSMutableArray *item = [NSMutableArray array];
    NSString *tempString;
    //拼音分组
...
+(NSMutableArray*)LetterSortSupplierModel:(NSArray*)stringArr
{
    ...
    NSString *tempString;
    //拼音分组
    for (SuppliersModel *object in tempArray) {
...
+(NSMutableArray*)ReturnSortChineseArr:(NSMutableArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ClientsModel *model = [stringArr objectAtIndex:i];
        ChineseContenr *chineseString = [[ChineseContenr alloc]init];
...
//                DDLog(@"%@",chineseString.string);
        ...
        model.pinYin = chineseString.pinYin;
        [chineseStringsArray addObject:model];
    }
    //按照拼音首字母对这些Strings进行排序
    NSArray *sortDescriptors = [NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"pinYin" ascending:YES]];
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        [result addObject:((ChineseContenr*)[tempArray objectAtIndex:i]).string];
        NSLog(@"SortArray----->%@",((ChineseContenr*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
